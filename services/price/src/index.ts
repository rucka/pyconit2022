...
import { authenticate, authorize, parseToken, plugins, preflight } from '@pyconit2022/ts-shared/src/http'
import { getPrice, updatePrice } from './price'
import { sign } from '@pyconit2022/ts-shared/src/jwt'
...
server.get('/price', parseToken, authenticate, (_, res) => {
...
server.post('/price', parseToken, authorize('admin', 'price'), (req, res) => {
  if (!req.body) {
